...
int CEFGlareBow::CheckFrame(void)
{
...
void CEFGlareBow::Init(void)
{
	...
	g_Particle.SetEffSetCon( 0.0f , EF_GLAER_BOW_0 , ESLT_CHARPOS , 
		EFFECT_SET_LOOP_TIME , m_pDestChr->GetUnique() , m_pDestChr->GetIndex() );
}

int	CEFGlareBow::Render(float dtime)
{
	// 활 날라 갈때
	if( m_ISShot )
	{
...
int CEFGlareBow::Update(float dtime)
{
...
int CEFGlareBow::SetAfterEffect(void)
{

	g_Particle.DelEffSetCon( EF_GLAER_BOW_0 , ESLT_CHARPOS, m_pDestChr->GetUnique(), m_pDestChr->GetIndex());	
	...
	switch( m_ID )
	{
		case EF_GLAER_BOW_0:
			...
			EffectID = EF_GLAER_BOW_ATTACKED_0;
				
			break;

		case EF_GLAER_BOW_1:

			EffectID = EF_GLAER_BOW_ATTACKED_1;

			break;

		case EF_GLAER_BOW_2:

			EffectID = EF_GLAER_BOW_ATTACKED_2;

			break;
	}

	g_Particle.SetEffSetCon( 0.0f , EffectID , 0 , 2.0f , 
