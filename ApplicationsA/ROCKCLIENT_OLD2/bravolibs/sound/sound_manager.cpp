...
//-----------------------------------------------------------------------------
// Name: CSoundManager::Release()
// Desc: release the class
// add : leechen 
//-----------------------------------------------------------------------------
...
//       then:   dwPrimaryChannels = 2  by simwoosung mono채널만이 3D사운드가 적용된다 - 1
//               dwPrimaryFreq     = 22050, 
//               dwPrimaryBitRate  = 16
//-----------------------------------------------------------------------------
                               ...
                               DWORD dwNumBuffers )
{
    ...
    apDSBuffer = SAFE_NEW_ARRAY( LPDIRECTSOUNDBUFFER , dwNumBuffers );	
    if( apDSBuffer == NULL )
    {
        hr = E_OUTOFMEMORY;
        goto LFail;
...
//            hr;//DXTRACE_ERR( TEXT("CreateSoundBuffer"), hr );
    ...
    for( i=1; i<dwNumBuffers; i++ )
    {
        ...
        {
//            hr;//DXTRACE_ERR( TEXT("DuplicateSoundBuffer"), hr );
            goto LFail;
        }
    }

    // Create the sound
    *ppSound = SAFE_NEW( CSound( apDSBuffer, dwDSBufferSize, dwNumBuffers, pWaveFile ) );	
...
// Desc: 
//-----------------------------------------------------------------------------
HRESULT CSoundManager::CreateFromHandle( CSound** ppSound, 
                               HANDLE hFile, 
                               DWORD dwCreationFlags, 
                               GUID guid3DAlgorithm,
                               DWORD dwNumBuffers )
{
    ...
    if( m_pDS == NULL )
        return CO_E_NOTINITIALIZED;
    if( hFile == NULL || ppSound == NULL || dwNumBuffers < 1 )
        return E_INVALIDARG;
    ...
    pWaveFile->OpenFromHandle( hFile, NULL, WAVEFILE_READ );
    ...
    if( FAILED( hr = m_pDS->CreateSoundBuffer( &dsbd, &apDSBuffer[0], NULL ) ) )
    {
		...
		switch( hr )
		{
			case DSERR_ALLOCATED:
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_ALLOCATED\n");
				break;
			case DSERR_CONTROLUNAVAIL:
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_CONTROLUNAVAIL\n");
				break;
			case DSERR_BADFORMAT:
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_BADFORMAT\n");
				break;
			case DSERR_BUFFERTOOSMALL:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_BUFFERTOOSMALL\n");
				break;
			case DSERR_DS8_REQUIRED:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_DS8_REQUIRED\n");
				break;
			case DSERR_INVALIDCALL:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_INVALIDCALL\n");
				break;
			case DSERR_INVALIDPARAM:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_INVALIDPARAM\n");
				break;
			case DSERR_NOAGGREGATION:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_NOAGGREGATION\n");
				break;
			case DSERR_OUTOFMEMORY:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_OUTOFMEMORY\n");
				break;
			case DSERR_UNINITIALIZED:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_UNINITIALIZED\n");
				break;
			case DSERR_UNSUPPORTED:  
				DebugPrint( "CSoundManager::CreateFromMemory DSERR_UNSUPPORTED\n");
				break;
                                        ...
                                        HANDLE hNotifyEvent )
{
    ...
    aPosNotify = SAFE_NEW_ARRAY( DSBPOSITIONNOTIFY , dwNotifyCount );	
    if( aPosNotify == NULL )
    ...
    // Create the sound
    *ppStreamingSound = SAFE_NEW( CStreamingSound( pDSBuffer, dwDSBufferSize, pWaveFile, dwNotifySize ) );
...
//-----------------------------------------------------------------------------
// Name: CSoundManager::CreateStreaming()
// Desc: CreateStreaming from opened file handle
//-----------------------------------------------------------------------------
HRESULT CSoundManager::CreateStreamingFromHandle( CStreamingSound** ppStreamingSound, 
                                        HANDLE hFile, 
                                        DWORD dwCreationFlags, 
                                        GUID guid3DAlgorithm,
                                        DWORD dwNotifyCount, 
                                        ...
                                        HANDLE hNotifyEvent )
{
    ...
    if( m_pDS == NULL )
        return CO_E_NOTINITIALIZED;
    if( hFile == NULL || ppStreamingSound == NULL || hNotifyEvent == NULL )
        return E_INVALIDARG;
                ...
                DWORD dwNumBuffers, CWaveFile* pWaveFile )
{
    ...
    m_apDSBuffer = SAFE_NEW_ARRAY( LPDIRECTSOUNDBUFFER , dwNumBuffers );	
...
LPDIRECTSOUNDBUFFER CSound::GetFreeBuffer()
{
    ...
    for( i = 0; i < m_dwNumBuffers; ++i )
    {
        if( m_apDSBuffer[i] )
        {  
            if( DS_OK == m_apDSBuffer[i]->GetStatus( &dwStatus ) )
			{
    ...
    if( i < m_dwNumBuffers )
	{
		//DebugPrint( "CSound::GetFreeBuffer = %d\n" , i );
    
		return m_apDSBuffer[ i ];
	}
    else
	{
		DebugPrint( "CSound::GetFreeBuffer = Random\n" );
...
//---------------------------------- -------------------------------------------
HRESULT CSound::Play( DWORD dwPriority, DWORD dwFlags, LONG dwVol )
{
    HRESULT hr;
    BOOL    bRestored;
	...
	switch( ( hr = m_apDSBuffer[0]->SetVolume( ( LONG )g_Sound.m_WaveRealVolume ) ) )
	{
		case DSERR_CONTROLUNAVAIL:			
			DebugPrint( "CSound::Play [ SetVolume ] = DSERR_CONTROLUNAVAIL\n" );
			
			break;

		case DSERR_GENERIC:  
			DebugPrint( "CSound::Play [ SetVolume ] = DSERR_GENERIC\n" );

			break;

		case DSERR_INVALIDPARAM:
			DebugPrint( "CSound::Play [ SetVolume ] = DSERR_INVALIDPARAM\n" );

			break;

		case DSERR_PRIOLEVELNEEDED:  
			DebugPrint( "CSound::Play [ SetVolume ] = DSERR_PRIOLEVELNEEDED\n" );
			
			break;

		case DS_OK: 
			//DebugPrint( "CSound::Play [ SetVolume ] = DS_OK\n" );

			break;
	}

	//pDSB->SetVolume ( dwVol );    

	//return 	pDSB->Play( 0, dwPriority, dwFlags );
	return 	m_apDSBuffer[0]->Play( 0, dwPriority, dwFlags );
...
CWaveFile::CWaveFile()
{
	...
	m_hFile					= NULL;
	m_bIsReadingFromHandle	= FALSE;
	m_dwFilePointWhenOpen	= 0;
}
...
HRESULT CWaveFile::Open( LPTSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        if( NULL == m_hmmio )
        {
            ...
            CHAR* pData = SAFE_NEW_ARRAY( CHAR , dwSize );			
...
//-----------------------------------------------------------------------------
// Name: CWaveFile::OpenFromHandle()
// Desc: Opens a wave file from opend file handle
//-----------------------------------------------------------------------------
HRESULT CWaveFile::OpenFromHandle( HANDLE hFile, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    HRESULT hr;

    m_dwFlags = dwFlags;
    m_bIsReadingFromMemory = FALSE;
	m_bIsReadingFromHandle = TRUE;

	// 오픈할때 화일 포인트를 기억 해 놓구 Close에서 다시 세팅해 놓는다.
	if ( hFile == NULL )
		return E_INVALIDARG;
	m_hFile = hFile;
	LONG lMove=0;
	m_dwFilePointWhenOpen = SetFilePointer( hFile, lMove, NULL, FILE_CURRENT );
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        if( NULL == m_hmmio )
			return E_FAIL;//DXTRACE_ERR( TEXT("mmioOpen from Handle"), E_FAIL );
    ...
    {
        m_hmmio = mmioOpen( NULL, &mmioInfo, MMIO_ALLOCBUF  | 
											 MMIO_READWRITE);
        if( NULL == m_hmmio )
            return E_FAIL;//DXTRACE_ERR( TEXT("mmioOpen"), E_FAIL );
...
HRESULT CWaveFile::ReadMMIO()
{
    ...
    {
        m_pwfx = (WAVEFORMATEX*)SAFE_NEW_ARRAY( CHAR , sizeof(WAVEFORMATEX) );		
        if( NULL == m_pwfx )
    ...
    else
    {
        ...
        m_pwfx = (WAVEFORMATEX*)SAFE_NEW_ARRAY( CHAR , sizeof(WAVEFORMATEX) + cbExtraBytes );		
        if( NULL == m_pwfx )
...
HRESULT CWaveFile::Close()
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
		if(m_bIsReadingFromHandle)
		{
	        mmioClose( m_hmmio, MMIO_FHOPEN );
			// 오픈할 때의 화일 포인트로 돌려 놓는다.
			SetFilePointer( m_hFile, m_dwFilePointWhenOpen, NULL, FILE_BEGIN );
		}
		else
	        mmioClose( m_hmmio, 0 );
        m_hmmio = NULL;
