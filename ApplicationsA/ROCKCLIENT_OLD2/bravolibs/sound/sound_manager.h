...
public:
    ...
    HRESULT CreateFromHandle( CSound** ppSound, HANDLE hFile, DWORD dwCreationFlags = 0, GUID guid3DAlgorithm = GUID_NULL, DWORD dwNumBuffers = 1 );
    ...
    HRESULT CreateStreamingFromHandle( CStreamingSound** ppStreamingSound, HANDLE hFile, DWORD dwCreationFlags, GUID guid3DAlgorithm, DWORD dwNotifyCount, DWORD dwNotifySize, HANDLE hNotifyEvent );
};
...
public:
    ...
    HRESULT Play( DWORD dwPriority=0, DWORD dwFlags=0, LONG dwVol = DSBVOLUME_MAX );
    HRESULT Stop();
    HRESULT Reset();
    BOOL    IsSoundPlaying();
};
...
public:
	...
	HANDLE		  m_hFile;
	BOOL		  m_bIsReadingFromHandle;
	DWORD		  m_dwFilePointWhenOpen;
...
public:
	...
	HRESULT OpenFromHandle( HANDLE hFile, WAVEFORMATEX* pwfx, DWORD dwFlags );
