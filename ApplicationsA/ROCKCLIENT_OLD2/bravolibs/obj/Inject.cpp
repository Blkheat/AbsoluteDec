...
DWORD GetPIDFromNamePSAPI(char *szProcessName)
{
	...
	do
	{
		...
		SAFE_DELETE(PIDArray);

//		PIDArray = malloc(cbAlloc);
		PIDArray = SAFE_NEW( DWORD( cbAlloc ) );
...
int InjectDll()
{
	...
	if (!GetModuleFileName(g_hInstance, szLibPath, MAX_PATH))
		return 0;
	
	int nLen = strlen( strrchr(szLibPath, '\\') + 1 );
	
	strncpy(strrchr(szLibPath, '\\') + 1 , szDllName, nLen);
...
int InjectCode ()
{
	...
	__try 
	{
		...
		// Wait for GetSASWtnd() to terminate and get return code (SAS Wnd handle)
		WaitForSingleObject(hThread, INFINITE);
