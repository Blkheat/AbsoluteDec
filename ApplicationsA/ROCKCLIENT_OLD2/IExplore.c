...
long DisplayHTMLPage(HWND hwnd, wchar_t *addr, TCHAR *post)
{
	IWebBrowser2	*webBrowser2;
	IOleObject		*browserObject;
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
	{
		...
		bstrURL = SysAllocString( addr );	//
		//if (!bstrURL){ goto Error; }
		...
		// PostData
		len= strlen(post);
		pSafeArray=SafeArrayCreateVector(VT_UI1, 0, len);
		//if(!pSafeArray) { goto Error; }
		if(SafeArrayAccessData(pSafeArray, (void **)&value)!=S_OK) { }//goto Error; }
		for(i=0; i<len; i++) value[i]=(char)post[i];
			
		//if(SafeArrayUnaccessData(pSafeArray)!=S_OK) { goto Error; }
			
		V_VT(&vtPostData)= VT_ARRAY|VT_UI1;
		V_ARRAY(&vtPostData)=pSafeArray;	
		
		
		//COleVariant szURL= "http://www.abcd.com";

		webBrowser2->lpVtbl->Navigate2(webBrowser2, &vURL,  &vFlags, &vTargetFrameName, &vtPostData, &vHeaders );
...
long DisplayHTMLPageW(HWND hwnd, wchar_t *addr, TCHAR *post)
{
	IWebBrowser2	*webBrowser2;
	IOleObject		*browserObject;
    BSTR			bstrURL = NULL, bstrHeaders = NULL;
	...
	wchar_t *value = NULL;
	SAFEARRAY *pSafeArray;
	wchar_t* pwcharpost = NULL; 
	wchar_t wtmpStr[512] ={0,};
	...
	// we initially attached the browser object to this window.
	browserObject = *((IOleObject **)GetWindowLongW(hwnd, GWL_USERDATA));
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
	{
		...
		bstrURL = SysAllocString( L"http://decomalltest.unalis.com.tw:8080/index.asp?id=woohee&check=fa60b9f1375baac167aefe1cf16b18be&Server=web");	//
		//if (!bstrURL){ goto Error; }
		...
		V_VT(&vHeaders)		= VT_BSTR;
		V_BSTR(&vHeaders)	= bstrHeaders;
		V_VT(&vboolCheck) = VT_BOOL;
		
		// PostData
		//len= wcslen( (wchar_t*)post );
		
	
	
		//strcpy(ctmpStr , "id=woohee&check=fa60b9f1375baac167aefe1cf16b18be&Server=web" );
		//len = strlen(ctmpStr);

		wcscpy(wtmpStr , L"id=woohee&check=fa60b9f1375baac167aefe1cf16b18be&Server=web");
		len= wcslen( wtmpStr )  ;

		pSafeArray=SafeArrayCreateVector(VT_UI1, 0, 1024);
		//if(!pSafeArray) { goto Error; }
	
		//if(SafeArrayAccessData(pSafeArray, (void **)&cvalue)!=S_OK) { }//goto Error; }
		//for(i=0; i<len; i++) cvalue[i]= ctmpStr[i];
		
		if(SafeArrayAccessData(pSafeArray, (void **)&value)!=S_OK) { }//goto Error; }
	//	for(i=0; i<len; i++) value[i]= wtmpStr[i];
	...
	if (!OleCreate(&CLSID_WebBrowser, &IID_IOleObject, OLERENDER_DRAW, 0, (IOleClientSite *)_iOleClientSiteEx, &MyIStorage, (void**)&browserObject))
	{
		...
		*((IOleObject **)ptr) = browserObject;
			...
			!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2))
		{
			...
			webBrowser2->lpVtbl->put_Resizable(webBrowser2,VARIANT_FALSE); // °¢ ¼³Á¤
			webBrowser2->lpVtbl->put_ToolBar(webBrowser2,VARIANT_FALSE);
			webBrowser2->lpVtbl->put_StatusBar(webBrowser2,VARIANT_FALSE);
			webBrowser2->lpVtbl->put_AddressBar(webBrowser2,VARIANT_FALSE);
			webBrowser2->lpVtbl->put_MenuBar(webBrowser2,VARIANT_FALSE);
			webBrowser2->lpVtbl->put_FullScreen(webBrowser2,VARIANT_TRUE); 
...
LRESULT CALLBACK WebWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg == WM_CREATE)
	{
